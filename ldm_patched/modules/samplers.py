...
def calc_cond_uncond_batch(model, cond, uncond, x_in, timestep, model_options):
    ...
    while len(to_run) > 0:
        ...
        transformer_options["cond_mark"] = compute_cond_mark(
            cond_or_uncond=cond_or_uncond, sigmas=timestep
        )
        transformer_options["cond_indices"], transformer_options["uncond_indices"] = (
            compute_cond_indices(cond_or_uncond=cond_or_uncond, sigmas=timestep)
        )
        ...
        if control is not None:
            ...
            control_cond = (
                c.copy()
            )  # get_control may change items in this dict, so we need to copy it
            c["control"] = control.get_control(
                input_x, timestep_, control_cond, len(cond_or_uncond)
            )
...
def sampling_function(
    ...
    edit_strength = sum(
        (item["strength"] if "strength" in item else 1) for item in cond
    )
